...
def confFile():
    ...
    if platform.system() == 'Windows':
        folder = os.path.join(os.path.join(os.environ['APPDATA'], 'YaCoin'))
    else:
        if platform.system() == 'Darwin':
            folder = os.path.expanduser('~/Library/Application Support/YaCoin/')
        else:
            folder = os.path.expanduser('~/.yacoin')

    return os.path.join(folder, 'yacoin.conf')

conf_path = confFile()
if not os.path.exists(conf_path):
    parser.error('''Yacoin configuration file not found. Manually enter your RPC password.\r\n'''
        ...
        '''rpcpassword=%x\r\n'''
        '''\r\n'''
        '''Keep that password secret! After creating the file, restart Yacoin.''' % (conf_path, random.randrange(2**128)))
...
for node in json.loads(data.decode()):
    ...
    try:
        ...
        if e.errno == errno.ECONNREFUSED:
            print ('Unable to communicate with Yacoin RPC')
        break
