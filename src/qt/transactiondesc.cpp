...
QString TransactionDesc::FormatTxStatus(const CWalletTx& wtx)
{
    ...
    {        
        TransactionRecord nc;
        int NumConf = (IsYACOIN045 ? nc.NumConfirmations : 6);        // 6 for old clients until switch
        int nDepth = wtx.GetDepthInMainChain();
        if (GetAdjustedTime() - wtx.nTimeReceived > 2 * 60 && wtx.GetRequestCount() == 0)
            return tr("%1/offline").arg(nDepth);
        else if (nDepth < NumConf)
...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
...

    {
        ...
        {
            strHTML += "<b>" + tr("Source") + ":</b> " + tr(wtx.IsCoinStake() ?"Minted" : "Mined") + "<br>";
        }
        ...
        if (wtx.IsCoinStake())
            strHTML += "<br>" + tr("Minted coins must mature") + (fTestNet ? " 52" : " 520") + (" blocks before they can be spent. When you generated this block, it was broadcast to the network to be added to the block chain. If it fails to get into the chain, its state will change to \"not accepted\" and it won't be spendable. This may occasionally happen if another node generates a block within a few seconds of yours.") + "<br>";

        if ( wtx.IsCoinBase())
            strHTML += "<br>" + tr("Mined coins must mature 30 blocks before they can be spent. When you generated this block, it was broadcast to the network to be added to the block chain. If it fails to get into the chain, its state will change to \"not accepted\" and it won't be spendable. This may occasionally happen if another node generates a block within a few seconds of yours.") + "<br>";
