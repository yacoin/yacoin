...
void BitcoinGUI::createActions()
{
    ...
    secondAuthAction->setStatusTip(tr("Second auth with your YaCoin addresses"));
    ...
    timer->start(nMillisecondsPerSecond); // check every second
...
void BitcoinGUI::createMenuBar()
{
    ...
    securityMenu->addAction(unlockWalletMiningAction); // We'll leave this here too since this is the normal space
    ...
    mining->addAction(unlockWalletMiningAction);
    mining->addAction(minerAction);

    QMenu *explorer = appMenuBar->addMenu(tr("&Explorer"));
    explorer->addAction(explorerAction);
}
...
void BitcoinGUI::mineYacoins()
{
     ...
     * item fuctionality. It's safe to assume that running dHashesPerSec indicates POW mining was
     * started.******************Enjoy the ensuing logic headache ;) ***************************/

    if ( !minerAction->isEnabled() && (dHashesPerSec == 0) && (powminer != POW_RPCRUNNING) )
            return;
    ...
    if ( (powminer == POW_ENABLED) && (!minerAction->isChecked()) )
    {
          powminer = POW_SHUTDOWN;
          minerAction->setStatusTip(tr("CPU MINER SHUTTING DOWN"));
          dHashesPerSec = 0;
    }    

    if ( (dHashesPerSec != 0) && (!minerAction->isChecked()) &&
         (powminer != POW_SHUTDOWN || powminer != POW_OFF || powminer != POW_ENABLED) )
    {
        // User is already RPC mining
    ...
    if ( (dHashesPerSec == 0) && (minerAction->isChecked()))
    {
         ...
         dHashesPerSec = 1.0; // Fixes issue where enable and disable right away
    }    
        ...
        case POW_SHUTDOWN:
                    GenerateYacoins(false, pwalletMain);
                    mapArgs["-gen"] = ("0");
                    sleep(3);
                   // minerAction->setChecked(false);
                    dHashesPerSec = 0;
                    powminer = POW_OFF;
                    return;

        case POW_ENABLED:
                    GenerateYacoins(true, pwalletMain);
                    mapArgs["-gen"] = ("1");
                    powminer = POW_ENABLED;
                    minerAction->setStatusTip(tr("CPU MINER RUNNING"));
                    break;

        case POW_RPCRUNNING: case POW_OFF: case POW_ERROR:
                    break;
        }
...
void BitcoinGUI::setEncryptionStatus(int status)
{
    ...
    case WalletModel::Unlocked:
        ...
        labelEncryptionIcon->setToolTip(fWalletUnlockMintOnly? tr("Wallet is <b>encrypted</b> and currently <b>unlocked for mining and minting</b>") : tr("Wallet is <b>encrypted</b> and currently <b>unlocked</b>"));
