                              ...
                              )
{
    ...
    for (unsigned int i = 0; i < nVIsize; ++i)
    {
    ...
    for (unsigned int i = 0; i < nVOsize; ++i)
    {
...
void ExplorerPage::setNumBlocks( int currentHeight )  
{
    ...
    //if(pclientModel)
    {
        QModelIndex     // an index into the block Model of the explorer page
            BlockTableIndex;
        int 
            nHOURS_AGO = 12;    // the time before which no blocks are shown
        const int
        ...
        uint256 
            theDefiningBlockHash = pblockindex->GetHash();  
        ...
        if ( nDeltaSecondsAgeOfBlock < ( nHOURS_AGO * nSECONDsPerHOUR ) )   // OK to update explorer window
        {   // now at 1 minute/ block, 12 hours is ~720 blocks
            QVector< QString > 
            ...
            block.ReadFromDisk(pblockindex, true); // true means read Tx's, false means don't
            ...
            {                               // by putting the model into the view
                ...
                if( !fDoneIt )  // we should only resize once, I would think?
                {
                    fDoneIt = true;
...
//
//_____________________________________________________________________________
TransactionExplorerPage::TransactionExplorerPage(QWidget *parent ) :
    QDialog(parent)
