...
//    dbenv.log_set_config(DB_LOG_AUTO_REMOVE, 1);
    ...
    if (!dbenv.get_lk_max_locks(&nMaxLocks))
    {
        ...
        {
            if (nBlocks < 1)
                strMessage = strprintf(_("Warning: DB_CONFIG has set_lk_max_locks %lu, which may be too low for a single block. If this limit is reached, YACoin may stop working."), (unsigned long)nMaxLocks);
            else
                strMessage = strprintf(_("Warning: DB_CONFIG has set_lk_max_locks %lu, which may be too low for a common blockchain reorganization. If this limit is reached, YACoin may stop working."), (unsigned long)nMaxLocks);
...
bool CTxDB::LoadBlockIndex()
{
    ...
    for (CBlockIndex* pindex = pindexBest; pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        {
            printf(
                    "LoadBlockIndex() : *** found bad block at %d, "
                    "hash=%s"
                    "\n"
                    "", 
                    pindex->nHeight, 
...
#ifdef _MSC_VER
            if (fPrintToConsole) 
                (void)printf( "Verifying %7d at 0, 1",
                              nCheckDepth - nCount  
                            );        
#endif        
                ...
                if (ReadTxIndex(hashTx, txindex))
                {
                    ...
                    if (nCheckLevel>2 || pindex->nFile != txindex.pos.nFile || pindex->nBlockPos != txindex.pos.nBlockPos)
                    {
...
#ifdef _MSC_VER
                            if (fPrintToConsole) 
                                (void)printf( "Verifying %7d at 0, 1, 2, 3",
                                              nCheckDepth - nCount  
                                            );        
#endif        
                        }
                        else
                        {
                            ...
                            {
                                printf(
                                        "LoadBlockIndex(): *** invalid tx position for "
                                        "%s"
                                        "\n"
                                        "", 
                                        hashTx.ToString().c_str()
                        ...
                        BOOST_FOREACH(const CDiskTxPos &txpos, txindex.vSpent)
                        {
                            if (!txpos.IsNull())
                            {
                                pair<unsigned int, unsigned int> 
                                    posFind = make_pair(txpos.nFile, txpos.nBlockPos);

                                if (!mapBlockPos.count(posFind))
                                {
                                    printf(
                                            "LoadBlockIndex(): *** found bad spend at %d, hashBlock="
                                            "\n"
                                            "%s, hashTx="
                                            "\n"
                                            "%s"
...
#ifdef _MSC_VER
                                    if (fPrintToConsole) 
                                        (void)printf( "Verifying %7d at 0, 1, 2, 4",
                                                      nCheckDepth - nCount  
                                                    );        
#endif        
                                }
                                ...
                                if (nCheckLevel>5)
                                {
                                    ...
                                    {
                                        printf(
                                                "LoadBlockIndex(): *** cannot read spending transaction of "
                                                "\n"
                                                "%s:%i from disk"
                                                "\n"
                                                "", 
...
#ifdef _MSC_VER
                                if (fPrintToConsole) 
                                    (void)printf( "Verifying %7d at 0, 1, 2, 4, 6",
                                                  nCheckDepth - nCount  
                                                );        
#endif        
                                    }
                    ...
                    BOOST_FOREACH(const CTxIn &txin, tx.vin)
                    {
                        ...
                        if (ReadTxIndex(txin.prevout.hash, txindex))
                        {
                            if (txindex.vSpent.size()-1 < txin.prevout.n || txindex.vSpent[txin.prevout.n].IsNull())
                            {
                                ...
                                // this will mess up the output counting!
#ifdef _MSC_VER
                                if (fPrintToConsole) 
                                    (void)printf( "Verifying %7d at 0, 1, 2, 5",
                                                  nCheckDepth - nCount  
                                                );        
#endif        
                                pindexFork = pindex->pprev;
...
#ifdef _MSC_VER
                if( 0 == ( nCount % 1000 ) )  // every 1000th time through the loop
                {
                    ...
                    if( 0 != nCount )
                    {
                        if (fPrintToConsole)
                            (void)printf( 
                                        ...
                                        floorf( float(nCount * 10000.0 / nMaxHeightGuess) ) / 100
                                        );
                    }
                }
                ++nCount;
