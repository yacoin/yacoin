...
bool CheckStake(CValidationState& state, CBlock* pblock, CWallet& wallet) {
  ...
  if (!CheckProofOfStake(state, pblock->vtx[1], pblock->nBits, proofHash,
                         hashTarget))
...
void StakeMinter(CWallet* pwallet) {
  ...
  while (true) {
    if (fShutdown || (chainActive.Tip() && chainActive.Tip()->nHeight + 1 >=
    ...
    if (pindexPrev->IsProofOfStake() &&  // which was, if last block was a PoS
...
static void YacoinMiner(CWallet* pwallet)  // here fProofOfStake is always false
{
  ...
  while (fGenerateBitcoins && nBlocksToGenerate != 0) {
    ...
    printf("Running YACoinMiner with %" PRIszu
           " transaction"
           "%s"
           " in block (%u bytes)"
           "\n"
...
#ifndef _MSC_VER
    (void)printf("Starting mining loop\n");
#endif
    while (fGenerateBitcoins && nBlocksToGenerate != 0) {
          ...
          if ((nNow - nHPSTimerStart) > (30 * nMillisecondsPerSecond)) {
                ...
                30)  // 30 seconds always true the first time!
            {
              ...
              if ((nStatisticsNumberOfBlocks > 0) &&
                ...
                printf("long average block period %" PRId64
                       " sec (divisor %d)\n"
...
#ifdef _MSC_VER
        //    #ifdef _DEBUG
        (void)printf("block drift too far behind, restarting miner.\n");
//    #endif
#endif
