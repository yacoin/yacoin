...
static bool fGenerateYacoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;
double dHashesPerSec;
int64_t nHPSTimerStart;
bool fPrintOnce = true;

void YacoinMiner(CWallet *pwallet)
{
//    void *scratchbuf = scrypt_buffer_alloc();
    ...
    while (fGenerateYacoins)
    {
        ...
        if (fPrintOnce)
            printf("Running YacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadYacoinMiner(void* parg)
{
    ...
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadYacoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
    ...
    if (fGenerate)
    {
        ...
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadYacoinMiner, pwallet))
                printf("Error: NewThread(ThreadYacoinMiner) failed\n");
            Sleep(10);
        }
    }
