...
CBigNum bnProofOfStakeLimit(~uint256(0) >> 24); // proof of stake target since the big bang.
CBigNum bnProofOfStakeHardLimit(~uint256(0) >> 30);
...
unsigned int nStakeTargetSpacingTestnet = 1 * 60; // 1-minute stake spacing
//unsigned int nStakeTargetSpacing = 1 * 60; // 1-minute stake spacing
unsigned int nModifierInterval = 6 * nOneHour; // time to elapse before new modifier is computed

int nCoinbaseMaturity = 500;

const ::int64_t
    nChainStartTime = 1367991200,           // Wed May 08 2013 05:33:20
...
int64_t nPingInterval = nOneHour / 2;

extern enum Checkpoints::CPMode CheckpointsMode;


/********PLANNED YACOIN 045 HARD FORK**********/

int64_t x045 = GetAdjustedTime();
int64_t y045 = YACOIN_045_SWITCH_TIME;

bool  IsYACOIN045 = (x045 >= y045);
...
unsigned char GetNfactor(::int64_t nTimestamp) 
{
    ...
    {
        printf(
                "GetNfactor: %"PRId64" -> %d %"PRId64" : %d / %d\n",
...
CBigNum inline GetProofOfStakeLimit(int nHeight, unsigned int nTime)
{
    if(fTestNet) // separate proof of stake target limit for testnet
        return bnProofOfStakeTestnetLimit;
    if(2 > 1) // true in most universes'
        return bnProofOfStakeHardLimit; // YAC has always been 30

    return bnProofOfWorkLimit; // return bnProofOfWorkLimit if in alt universe
}
...
int64_t GetProofOfWorkReward(unsigned int nBits, int64_t nFees)
{
    ...
    // YaCoin: subsidy is cut in half every 64x multiply of PoW difficulty
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, unsigned int nBits, int64_t nTime, bool bCoinYearOnly)
{
    ...
    // Rather than set arbitrary stake rewards, let's let nFactor determine.
    int nStakepercent = 0;
    nStakepercent = GetNfactor(pindexBest->GetBlockTime()) / 2;

    int64_t nRewardCoinYear, nSubsidy, nSubsidyLimit = nStakepercent * COIN;
...
static const int64_t nTargetSpacingWorkMax = 12 * nStakeTargetSpacing; // 12-minutes, old YAC.
...
{
    if(fTestNet)
        return nStakeTargetSpacingTestnet; // 1 minute on testNet

    if(nTime > TARGETS_SWITCH_TIME)
        return 30 * nStakeTargetSpacing; // 30 minutes on mainnet since YACOIN_045_SWITCH_TIME   

    return nStakeTargetSpacing; // 1 minutes otherwise
}
...
else
{
    ...
    CBigNum bnNew;
    bnNew.SetCompact(pindexPrev->nBits);
    int64_t nTargetSpacing = fProofOfStake? nStakeTargetSpacing : min(GetTargetSpacingWorkMax(pindexLast->nHeight, pindexLast->nTime), (int64_t) nStakeTargetSpacing * (1 + nPoSBlocksBetween));
    int64_t nInterval = nTargetTimespan / nTargetSpacing;
    bnNew *= ((nInterval - 1) * nTargetSpacing + nActualSpacing + nActualSpacing);
    bnNew /= ((nInterval + 1) * nTargetSpacing);
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in YaCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    else
    {
        ...
        if (!IsYACOIN045 && !vchBlockSig.empty())
       {       
        // Yacoin: check proof-of-work block signature
        if (fCheckSig && !CheckBlockSignature())
            return DoS(100, error("CheckBlock() : bad 044 proof-of-work block signature"));
       }
...
::int32_t CBlockIndex::GetSpacingThreshold() const
{
    ...
    ::int32_t 
        spacing =  (0 < nPosBlockCount)?
                   (::int32_t)(round( GetPoWPoSRatio() - 0.75 ) ):
                   0;
    return std::max( spacing, 1 );
}

// YACOIN TODO OLD GBT
uint256 CBlockIndex::GetBlockTrustOld() const
{
    CBigNum bnTarget;
    bnTarget.SetCompact(nBits);
    if (bnTarget <= 0)
    ...
    if (nHeight >= nConsecutiveStakeSwitchHeight)
    {
        ...
        if (IsProofOfStake() && pprev->IsProofOfWork())
            return pprev->GetBlockTrustOld() + 1;
        ...
        if (IsProofOfWork())
        {
            ...
            if (pprev->IsProofOfStake())
                (bnTrust *= 2).getuint256();

            return bnTrust.getuint256();
        }

        // what the hell?!
        return error("GetBlockTrustOld : Something bad happened!");
    }
        // Genesis to block 420,000
        CBigNum bnOrigTrust = (CBigNum(1)<<256) / (bnTarget+1);
        return (IsProofOfStake() ? bnOrigTrust.getuint256() : 1);
}
...
bool CBlock::SignBlock(const CKeyStore& keystore)
{
    ...
    printf("Sign 044 POW block failed\n");
    return false;
}
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        *****************************************************************************************/
        ...
        // Genesis block
        const string strTimestamp = "https://bitcointalk.org/index.php?topic=196196";
        CTransaction txNew;
        ...
        txNew.vin[0].scriptSig =
            ...
            << vector<unsigned char>(strTimestamp.begin(), strTimestamp.end());
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion < MIN_BUGGY_VERSION)
        {
