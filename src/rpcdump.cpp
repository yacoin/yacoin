...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <yacoinprivkey> [label] [rescan=true]\n"
...
Value dumppem(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "dumppem <yacoinaddress> <filename> <passphrase>\n"
            "Dump the key pair corresponding to <yacoinaddress> and store it as encrypted PEM file."
            + HelpRequiringPassphrase());
