...
def main():
    ...
    tmpdir = "%s/yacoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and yacoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "yacoind"]) is not None:
    ...
    #Set env vars
    if "yacoinD" not in os.environ:
        os.environ["yacoinD"] = build_dir + '/src/yacoind' + exeext
        os.environ["yacoinCLI"] = build_dir + '/src/yacoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `yacoin-cli help` (`rpc_interface.txt`).
